...
def render_frame(source, timeinfo):
    i = timeinfo.global_frame
    gbl_time = timeinfo.get_global_time()

    p = subprocess.Popen(
        [
            batchRIshader,
            "--num_particles",          str(65536 << (13-3)),
            "--num_div_particles",      str(64),
            "--super_sampling_level",   str(1),
            "--num_tile_x",             str(1),
            "--num_tile_y",             str(1),
            ...
            "--output_h",               str(1080),
            "--hide_gl_info",           "on",
            "-D",                       "TIME=%f" % (gbl_time/timeinfo.duration_scaling),
            "tdf17bg.frag",
            "-",
            "particle_DF.frag"
        ],
...
def render_anim():
    ...
    for scn in scenes:
        ...
        for i in range(int(FPS*duration)):
            ...
            render_frame(scn[0](timeinfo), timeinfo)
            nfrm += 1
